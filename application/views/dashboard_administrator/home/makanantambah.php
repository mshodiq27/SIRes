  ...
  <div class="well">  
	<div class="navbar navbar-inverse">
	  <div class="navbar-inner">
		<div class="container">                
		  <a class="brand" href="#">Tambah Makanan</a>	          
		</div>
	  </div><!-- /navbar-inner -->
	</div><!-- /navbar -->      
    <section>
    <script type="text/javascript">
	function cekAddMakanan(){
		var nama = $('#nama').val();
		...
		if(nama.length < 1 || ket.length < 1 || status.length < 1 || harga.length < 1){
			...
			if(isNaN(status) || isNaN(harga)){
				...
				if(confirm('Anda yakin ingin menambahkan data makanan ini ?')){				
    ...
    <?php
	echo form_open_multipart('dashboard_administrator/makanan/tambah',array('class'=>'form-horizontal','id'=>'formadd','onsubmit'=>'cekAddMakanan();return false;'));
